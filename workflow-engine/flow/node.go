...
func (n *Node) GetProcessConfigFromJSONFile() {
	...
	if err != nil {
		log.Printf("GetProcessConfigFromJSONFile cannot open file processConfig.json:%v", err)
		panic(err)
	}
	decoder := json.NewDecoder(file)
	err = decoder.Decode(n)
	if err != nil {
		log.Printf("GetProcessConfigFromJSONFile decode processConfig.json failed:%v", err)
	}
}
