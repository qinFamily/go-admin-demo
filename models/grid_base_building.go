...
type GridBasicBuiding struct {
	BuildingId   int    `json:"buildingId" gorm:"type:bigint(20);primary_key"` // 建筑ID
	BuildingName string `json:"buildingName" gorm:"type:varchar(50)"`          // 建筑名称
	GridId       int    `json:"gridId" gorm:"type:bigint(20);"`                // 网格ID
	SecretaryId  int    `json:"secretaryId" gorm:"type:bigint(20);"`           // 楼栋长ID
	Sits         string `json:"sits" gorm:"type:tinytext;"`                    // 坐落地
	Structure    int    `json:"structure" gorm:"type:tinyint(4);"`             // 建筑结构 0-未知 1-钢混 2-木 3-砖 4-钢架构
	Layers       int    `json:"layers" gorm:"type:tinyint(4);"`                // 建筑层数
	Years        string `json:"years" gorm:"type:date;"`                       // 建筑年代
	RiskLevel    int    `json:"riskLevel" gorm:"type:tinyint(4);"`             // 建筑危险等级 0-未知 1-正常 2-轻度 3-中度 4-重度 5-危房 6-拆除
	Developers   string `json:"developers" gorm:"type:varchar(50);"`           // 开发商
