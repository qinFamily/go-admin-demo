...
func (e *TicketsTicket) Create() (TicketsTicket, error) {
	...
	if err != nil {
		wf = WorkflowsWorkflow{
			ID:             e.WorkflowId,
			TicketSnPrefix: "unknow",
		}
	}
	e.Sn = fmt.Sprintf("%s_%d", wf.TicketSnPrefix, time.Now().UnixNano())
...
// 获取TicketsTicket
func (e *TicketsTicket) Get(isReleated bool) (doc TicketsTicket, err error) {
	if e.CreateBy == 0 && len(e.DataScope) > 0 {
