...
type GridBasicHousing struct {
	HouseId    int    `json:"houseId" gorm:"type:bigint(20);primary_key"` // 房间ID
	BuildingId int    `json:"buildingId" gorm:"type:bigint(20);"`         // 楼栋ID
	Cell       string `json:"cell" gorm:"type:varchar(20);"`              // 单元号
	Number     string `json:"number" gorm:"type:varchar(20);"`            // 房间号
	StbMac     string `json:"stbMac" gorm:"type:varchar(20);"`            // 机顶盒MAC地址
	Type       int    `json:"type" gorm:"type:tinyint(4);"`               // 房屋类型,0-未知 1-正常 2-危房
	Purpose    int    `json:"purpose" gorm:"type:tinyint(4);"`            // 房屋用途，0-空置 1-自住 2-厂房 3-公寓 4-出租
	PhoneNum   string `json:"phoneNum" gorm:"type:varchar(13);"`          // 联系电话
	OwnerId    int    `json:"ownerId" gorm:"type:bigint(20);"`            // 产权人ID
	UserId     int    `json:"userId" gorm:"type:bigint(20);"`             // 使用人
	Area       string `json:"area" gorm:"type:float;"`                    // 建筑面积
	PeopleNum  int    `json:"peopleNum" gorm:"type:tinyint(4);"`          // 居住人口
	Remarks    string `json:"remarks" gorm:"type:tinytext;"`              // 备注
